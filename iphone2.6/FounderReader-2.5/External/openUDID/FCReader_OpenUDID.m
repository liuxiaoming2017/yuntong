...
@interface FCReader_OpenUDID (Private)
...
@implementation FCReader_OpenUDID
...
# error                                                         ^ read comments above, fix accordingly, and remove this #error line
    ...
    // both space and time by combining a value unique to the computer on which it was generated¡ªusually the
    // Ethernet hardware address¡ªand a value representing the number of 100-nanosecond intervals since 
...
//
+ (NSString*) value {
    return [FCReader_OpenUDID valueWithError:nil];
}
...
// zhe ju hua shi zi dong xiu zheng de ,ru guo cuo le ,qing zi xing xiu zheng ,yuan lai shi shang mian na yi ju .
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [FCReader_OpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [FCReader_OpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [FCReader_OpenUDID _setDict:localDict forPasteboard:slotPB];
    }
