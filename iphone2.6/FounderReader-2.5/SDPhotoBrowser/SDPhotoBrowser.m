...
//    saveButton.clipsToBounds = YES;
    saveButton.frame = CGRectMake(kSWidth-50, kSHeight-50, 32, 32);
    [saveButton setImage:[UIImage imageNamed:@"toolbar_save_nor"] forState:UIControlStateNormal];
    [saveButton setImage:[UIImage imageNamed:@"toolbar_save_pre"] forState:UIControlStateHighlighted];
...
- (void)photoClick:(UITapGestureRecognizer *)recognizer
{
    ...
    // 个性化：
    // 当已经放大的第x张图片要缩小时，x不属于展示小图片的imageView的个数时，不走按frame缩小的动画代码，直接删除放大的图片view
    if (self.displayedPhotoCount && currentIndex >= self.displayedPhotoCount) {
