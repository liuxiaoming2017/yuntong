    ...
    [self.slider mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        if (kScreenWidth<kScreenHeight) {
            ...
            if(self.isFullScreen){
                make.width.mas_equalTo(kScreenHeight - padding - 60 - 50 - 30 - padding - padding);
            }else{
            make.width.mas_equalTo(kScreenWidth - padding - 60 - 50 - 30 - padding - padding);
            }
        }
    }];
...
-(void)closeBtnAction:(UIButton *)button{
    if([self.delegate respondsToSelector:@selector(controlView:withCloseButton:)]){
        [self.delegate controlView:self withCloseButton:button];
    }
}
