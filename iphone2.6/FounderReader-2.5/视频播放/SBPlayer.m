...
//  SBPlayer
//
//  Created by sycf_ios on 2017/28/11.
...
-(void)addNotificationCenter{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(audioRouteChangeListenerCallback:)  name:AVAudioSessionRouteChangeNotification object:nil];//耳机插入拔出通知
}
- (void)audioRouteChangeListenerCallback:(NSNotification*)notification
{
    ...
    switch (routeChangeReason) {
        ...
        case AVAudioSessionRouteChangeReasonOldDeviceUnavailable:
            NSLog(@"耳机拔出，停止播放操作");
            dispatch_async(dispatch_get_main_queue(), ^{
                if(_playOrPauseButton.isSelected){
...
-(UIButton *)closeBtn{
    if(!_closeBtn){
        ...
        _closeBtn.contentMode = UIViewContentModeScaleToFill;
        [_closeBtn setImage:[UIImage imageNamed:@"video_close"] forState:UIControlStateNormal];
...
-(void)controlView:(SBControlView *)controlView withLargeButton:(UIButton *)button{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"fullScreenBtnClickNotice" object:button];
...
- (void)closeBtnAction
{
    if([self.delegate respondsToSelector:@selector(playerCloseAction)]){
        [self.delegate playerCloseAction];
    }
    [self stop];
    self.player = nil;
...
- (UIButton *)playOrPauseButton
{
    if (!_playOrPauseButton) {
        ...
        [_playOrPauseButton setImage:[UIImage imageNamed:@"Video_play"] forState:UIControlStateNormal];
        [_playOrPauseButton setShowsTouchWhenHighlighted:YES];
        [_playOrPauseButton setImage:[UIImage imageNamed:@"video_pause"] forState:UIControlStateSelected];
