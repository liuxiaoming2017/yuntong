...
@interface HeaderNewsWidget()
{
    ...
    CGFloat headerScale;
    BOOL isShowReadCount;
    BOOL isShowPublishTime;
    BOOL isShowTag;
    CGFloat titleBgHeight;
    BOOL _isHideColumnReadCount;
}
...
-(id)initWithConfigDic:(NSDictionary *)dicInfo IsHideColumnReadCount:(BOOL)isHideColumnReadCount
{
    ...
    _isHideColumnReadCount = isHideColumnReadCount;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self){
        if([[AppConfig sharedAppConfig].sid compare:@"xjdcb"] == NSOrderedSame){
            //设置是否显示阅读数、发布时间
            isShowReadCount = NO;//[AppConfig sharedAppConfig].isAppearReadCount;
            isShowPublishTime = NO;
            //标题区域显示的高度，默认55，大晨报改为40
            titleBgHeight = 40;
        } else{
            //设置是否显示阅读数、发布时间
            if (![AppConfig sharedAppConfig].isCarouselAppearReadCount || ([AppConfig sharedAppConfig].isCarouselAppearReadCount && _isHideColumnReadCount))
                isShowReadCount = NO;
            else
                isShowReadCount = YES;
            isShowPublishTime = [AppConfig sharedAppConfig].isCarouselAppearDate;
            if (!isShowReadCount && !isShowPublishTime) {
                isShowTag = NO;
                titleBgHeight = 40;
            } else {
                isShowTag = YES;
...
- (void)addTitleAndPageControl
{
    ...
    titleBgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, kSWidth*headerScale-titleBgHeight, kSWidth, titleBgHeight)];
    ...
    titleLabel = [[Label alloc] initWithFrame:CGRectMake(8, kSWidth*headerScale-titleBgHeight, kSWidth-16, 40)];
    titleLabel.backgroundColor = [UIColor clearColor];
    titleLabel.edgeInsets = UIEdgeInsetsMake(0, 3, 0, 0);
    ...
    pubTimeLabel.hidden = !isShowPublishTime;
    ...
    if (_imageArray.count > 1)
    {
        ...
        if (article.advID == 0) {
            ...
            readLabel.hidden = !isShowReadCount;
            if ( article.articleType == ArticleType_LIVESHOW ) {
                if (_isHideColumnReadCount) {
                    readLabel.hidden = YES;
                }else{
                    if ([AppConfig sharedAppConfig].isCarouselLiveAppearReadCount) {
                        readLabel.hidden = NO;
                    }else{
                        readLabel.hidden = YES;
                    }
        ...
        if (isShowTag && article.tag != nil && ![article.tag isEqualToString:@""]) {
    ...
    if (self.isDotStyle == 0) {
        ...
        _stylePageControl = [[StyledPageControl alloc] initWithFrame:CGRectMake((kSWidth-8*_imageArray.count-7), kSWidth*headerScale-12, 8*_imageArray.count*proportion, 8)];
    ...
    for (int i = 0; i < _AllheaderArticles.count; i++)
    {
        ...
        if (article.advID != 0){//推广
            if ([article.imgAdvUrl containsString:@".gif"]) {
                ...
                imageurl = [NSString stringWithFormat:@"%@@!lg", article.imgAdvUrl];
            ...
            if ([article.imageUrlBig containsString:@".gif"]) {
                FLAnimatedImageView *imageView = [[FLAnimatedImageView alloc] initWithFrame:bigImageView.frame];
                imageView.image = [Global getBgImage43];
                ...
                imageurl = [article.imageUrl stringByReplacingOccurrencesOfString:@"sm43" withString:@"lg"];
                [bigImageView sd_setImageWithURL:[NSURL URLWithString:imageurl] placeholderImage:[Global getBgImage43]];
                [self.scrollView addSubview:bigImageView];
            }
        }
