...
static const CGFloat XHAnimationDampingDuration = 0.3f;
static const CGFloat XHAnimationVelocity = 0.f;
...
- (void)viewDidLoad {
    ...
    UIPanGestureRecognizer *gesture = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(contentContainerButtonPressed:)];
    [self.zoomDrawerView.contentContainerButton addGestureRecognizer:gesture];
    [gesture release];
...
- (void)closeDrawerAnimated:(BOOL)animated velocity:(CGFloat)velocity animationOptions:(UIViewAnimationOptions)options completion:(void (^)(BOOL finished))completion {
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 7.0) {
        [UIView animateWithDuration:self.animateDuration delay:0.0 usingSpringWithDamping:damping initialSpringVelocity:velocity options:UIViewAnimationOptionAllowAnimatedContent animations:^{
        ...
        [UIView animateWithDuration:self.animateDuration delay:0 options:options animations:^{
            [self.scrollView setContentOffset:CGPointMake(XHContentContainerViewOriginX, 0.0f) animated:NO];
        } completion:^(BOOL finished) {
            self.openSide = XHDrawerSideNone;
            self.zoomDrawerView.contentContainerButton.userInteractionEnabled = NO;
...
- (void)openDrawerSide:(XHDrawerSide)drawerSide animated:(BOOL)animated velocity:(CGFloat)velocity animationOptions:(UIViewAnimationOptions)options completion:(void (^)(BOOL finished))completion {
    ...
    CGFloat damping = [self isSpringAnimationOn] ? 0.8f : 1.0f;
    ...
    if ([[UIDevice currentDevice].systemVersion floatValue] >= 7.0) {
        [UIView animateWithDuration:self.animateDuration delay:0.0 usingSpringWithDamping:damping initialSpringVelocity:velocity options:UIViewAnimationOptionAllowAnimatedContent animations:^{
            if (drawerSide == XHDrawerSideLeft) {
                ...
                [self.scrollView setContentOffset:CGPointMake(2.2 * XHContentContainerViewOriginX, 0.0f) animated:NO];
            }
        } completion:^(BOOL finished) {
            self.openSide = drawerSide;
...
-(void)reloadNavBar{
    ...
    //在关于页面Home键退出返回后，主屏导航栏上移了20个像素，需要做处理
    UINavigationController *navController =  (UINavigationController *)(self.centerViewController);
    CGRect navrect = navController.navigationBar.frame;
    if (navrect.origin.y == 0 && navrect.size.height == 44.0 &&  (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7.0"))) {

        navrect.size.height = 64.0f;
        navController.navigationBar.frame = navrect;
        for (UIViewController * controller in navController.viewControllers) {
            
            CGRect rectview = controller.view.frame;
            rectview.origin.y = 0;
            rectview.size.height = [UIScreen mainScreen].bounds.size.height - navrect.size.height;
            controller.view.frame = rectview;
            
            if(controller.view.alpha == 1.0){ //只改变一次子视图的坐标位置
                controller.view.alpha = 0.99999;
                for (UIView *view1 in [controller.view subviews]) {
...
- (void)replaceController:(UIViewController *)oldController newController:(UIViewController *)newController container:(UIView *)container
{
    if (newController) {
        ...
        [[newController view] setFrame:rect];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (currentContentOffsetX > 0 && currentContentOffsetX < XHContentContainerViewOriginX && self.cuurrentContentOffsetX > currentContentOffsetX) {
        self.openSide = XHDrawerSideLeft;
    } else if (currentContentOffsetX > XHContentContainerViewOriginX && currentContentOffsetX < (XHContentContainerViewOriginX * 2.2) && self.cuurrentContentOffsetX < currentContentOffsetX) {
        self.openSide = XHDrawerSideRight;
    }
    
    CGFloat contentOffsetX = 0.0;
    if (self.openSide == XHDrawerSideRight) {
        contentOffsetX = XHContentContainerViewOriginX * 2.2 - contentOffset.x;
    } else if (self.openSide == XHDrawerSideLeft) {
        contentOffsetX = contentOffset.x;
    }
    
    
    CGFloat contentContainerScale = powf((contentOffsetX + XHContentContainerViewOriginX) / (XHContentContainerViewOriginX * 2.0f), .2f);
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    CGFloat drawerPadding = XHContentContainerViewOriginX * 2.2 / 3.;
    if ((targetContentOffsetX >= drawerPadding && targetContentOffsetX < XHContentContainerViewOriginX && self.openSide == XHDrawerSideLeft) || (targetContentOffsetX > XHContentContainerViewOriginX && targetContentOffsetX <= (XHContentContainerViewOriginX * 2.2 - drawerPadding) && self.openSide == XHDrawerSideRight)) {
        ...
        self.openSide = XHDrawerSideLeft;
    } else if ((targetContentOffsetX > (XHContentContainerViewOriginX * 2.2 - drawerPadding) && targetContentOffsetX <= (XHContentContainerViewOriginX * 2.2) && self.openSide == XHDrawerSideRight)) {
        targetContentOffset->x = XHContentContainerViewOriginX * 2.2;
        self.openSide = XHDrawerSideRight;
    }
}
