...
- (void)setDefaultSettings {
    ...
    menuContainerView = [[UIView alloc] init];
...
- (void)setMenuState:(MFSideMenuState)menuState {
    //guo ad
    if (menuState == MFSideMenuStateClosed) {
//        self.isLeftPan = YES;
    }else{
        self.isLeftPan = NO;
    }
...
- (void) handlePan:(UIPanGestureRecognizer *)recognizer {
    ...
    if(self.panDirection == MFSideMenuPanDirectionNone) {
        ...
        if(fabs(translatedPoint.y - panGestureOrigin.y) > 0 && fabs(translatedPoint.x - panGestureOrigin.x) > 0){
            self.panDirection = MFSideMenuPanDirectionNone;
            return;
        }
